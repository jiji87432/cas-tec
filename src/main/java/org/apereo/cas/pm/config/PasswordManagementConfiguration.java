...
public class PasswordManagementConfiguration {
	...
	public PasswordManagementService passwordChangeService() {
		...
		if (casProperties.getAuthn().getPm().isEnabled()) {
			if (StringUtils.isNotBlank(pm.getLdap().getLdapUrl()) && StringUtils.isNotBlank(pm.getLdap().getBaseDn())
					&& StringUtils.isNotBlank(pm.getLdap().getUserFilter())) {
				return new LdapPasswordManagementService(passwordManagementCipherExecutor(), casProperties.getServer().getPrefix(), casProperties.getAuthn().getPm());
			}

			if (StringUtils.isNotBlank(pm.getJdbc().getSqlChangePassword()) && StringUtils.isNotBlank(pm.getJdbc().getSqlFindEmail())
					&& StringUtils.isNotBlank(pm.getJdbc().getUrl()) && StringUtils.isNotBlank(pm.getJdbc().getUser())) {
				return new JdbcPasswordManagementService(passwordManagementCipherExecutor(), casProperties.getServer().getPrefix(),
						casProperties.getAuthn().getPm(), dataSource);
			}

			if (StringUtils.isNotBlank(pm.getRest().getEndpointUrlChange()) && StringUtils.isNotBlank(pm.getRest().getEndpointUrlEmail())) {
				return new RestPasswordManagementService(passwordManagementCipherExecutor(), casProperties.getServer().getPrefix(), new RestTemplate(),
